<template>
  <div>
    ...
    <div class="header_content">
      <img class="header_left" :style="{marginLeft:boolBack?'66px':'34px'}" src="../../assets/image/home/img_logo.png"
        alt="img">
      ...
      <div class="header_right">
          ...
          <div class="hover_bell_content" v-show="showBellContent">
            ...
            <div>
              <div class="flex_row" style="margin-bottom: 15px;">
                ...
                <p class="bell_item_span1">通知</p>
                <p class="bell_item_span2">(0)</p>
              </div>
              <div style="border-bottom: 1px solid #ECECEC;">
                <!-- <div class="item_words_box">
                  <p class="item_word">报告：您的12月运维月报已送达！</p>
                  <p class="item_words_time">2020-01-15 15:00:00</p>
                </div>
                <div class="item_words_box">
                  <p class="item_word">审核：贾豫提交的工单需要您审核！</p>
                  <p class="item_words_time">2020-01-16 15:00:00</p>
                </div> -->
              </div>
            </div>
            <div>
            ...
            <div>
              <div class="flex_row" style="margin: 15px 0;">
                ...
                <p class="bell_item_span1">报警</p>
                <p class="bell_item_span2">(0)</p>
              </div>
              <div>
                <!-- <div class="item_words_box">
                  <p class="item_word">一级告警：裕安大厦10kV 1段 101 裕安大厦啊哈哈哈</p>
                  <p class="item_words_time">2020-01-15 15:00:00</p>
                </div>
                <div class="item_words_box">
                  <p class="item_word">预警：裕安大厦10kV 22 裕安大厦啊哈哈哈</p>
                  <p class="item_words_time">2020-01-16 15:00:00</p>
                </div> -->
              </div>
            </div>
        ...
        <div class="flex_row" style="position:relative" @mouseover="mouseOverUserInfo" @mouseleave="mouseLeaveUserInfo">
          ...
          <div class="user_info_content" v-if="showUserName">
            ...
            <p @click="toPersonalSet" class="user_info_item">个人设置</p>
    ...
    <div class="main_content">
      <div :class="{left_side_expand: isExpand, left_side_shrink: !isExpand}">
        ...
        <div v-for="(menu, key) in menuList" :key="key" class="left_item" @click="toItem(menu)"
      ...
      <div class="right_side grey_bg" :style="{backgroundColor: this.currentPage=='AdminHome' ? '#021331' : '#f0f2f5'}">
        <!-- 标签功能模块 -->
        <div class="tag_box">
        ...
        <transition name="fade">
          <keep-alive v-if="isMyComponent" :exclude="excludeComponent">
            <component class="component_style" v-bind:is="currentPage">
            </component>
          </keep-alive>
          <iframe class="iframe_box" v-if="!isMyComponent" :src="pageSrc" scrolling="auto" ref="iframe"
    ...
    <div class="float_content" @mouseover="mouseOverFast" @mouseleave="mouseLeaveFast">
      <div class="hover_float_content" v-show="showFastContent">
        <!-- v-show="showFastContent"  -->
        <div>
          <span class="choose_label">选择公司</span>
          <select name="company-choice" v-model="companyId" class="common_choice">
            <option class="common_choice_item" v-for="item in headerClient" :key="item.value" :value="item.value">
              {{item.label}}</option>
          </select>
          <!-- @change="handleClient" -->
        ...
        <div style="margin-top: 20px;">
          ...
          <div class="item_fast_box">
            <div class="fast_btn" v-for="(item, key) in itemFastList" :key="key" @click="toMenuItem(item)">{{item.name}}
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      excludeComponent: ['bianyaqi', 'dianrongqi', 'duanluqi', 'dianfei', 'sunhao', 'monitorTo'],
      pageSrc: '',
      isNowTag: true,
      timer: null,
  ...
  created () {
    // this.isLogin()
    // this.timer = setInterval(this.userLoginKick, 5000)
  ...
  methods: {
    ...
    // userLoginKick () {
    //   const that = this
    //   that.$store.dispatch('userLoginKick', {}).then(res => {
    //     if (res && res.result.kickOut && typeof (res.result.kickOut) !== "string") {
    //       clearInterval(that.timer)
    //       window.localStorage.removeItem('userInfo')
    //       that.$alert('该账号在其他地方登录，被迫下线', '消息提示', {
    ...
    getQuickEntry () {
      const that = this
      let data = {
        // userId: '774bdc0f_27b9_42e2_adb5_ab2c00c67b79'
        userId: that.userInfo.userId
      }
      this.$store.dispatch('quickEntry', data).then(res => {
        if (res && res.result.quickEntry) {
          this.itemFastList = res.result.quickEntry
        }
      })
    },
    // 点击头部菜单
    ...
    toItem (item) {
      ...
      if (!item.href) {
        ...
        if (item.children && item.children.length == 0 && this.tagList.filter(i => i.href == item.href).length == 0) {
          let data = {
            name: item.name,
            routeName: item.routeName,
            href: item.href,
        ...
        let routeNum = this.tagList.findIndex(i => i.href === item.href)
        this.tagList[routeNum].isActive = true
      }
    ...
    async toMenuItem (item) {
      ...
      if (!item.href) {
        ...
        this.isMyComponent = false
        if (this.tagList.filter(i => i.href == item.href).length == 0) {
    ...
    async iframeToPage (event) {
      ...
      let newArr = await this.steamroller(this.menuJson)
    ...
    closeTag (item) {
      ...
      if (item.href) {
        ...
        // if (item.href == this.pageSrc) {
        let tagNum = this.tagList.length - 1
        if (this.tagList[tagNum].href) {
          this.isMyComponent = false
    ...
    toPersonalSet () {
      let data = {
        name: '个人设置',
        routeName: 'PersonalSet',
        href: 'https://e.intellqc.com/_base/user_set/mine.aspx'
      }
      this.toMenuItem(data)
    },
    // 移入
    ...
    closeNow () {
      ...
      if (this.currentPage == '' && this.pageSrc !== '') {
        this.tagList = this.tagList.filter(i => i.href !== this.pageSrc)
        let tagNum = this.tagList.length - 1
        if (this.tagList[tagNum].href) {
          this.isMyComponent = false
          this.pageSrc = this.tagList[tagNum].href
        ...
        this.tagList[tagNum].isActive = true
      } else if (this.currentPage !== '' && this.pageSrc == '') {
