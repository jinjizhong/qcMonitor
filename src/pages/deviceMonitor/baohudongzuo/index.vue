<template>
  <div>
    <topHeader :options="topInfo" @change="handleTopChange"></topHeader>
    <div :class="{ flex_row: $isPc, flex_column: !$isPc }">
      <deviceInfo deviceName="保护动作" runningStatus="0" communicateStatus="0" :deviceInfoList="equipAttr"
        @warnDataBtn="warnDataBtn"></deviceInfo>
        ...
        <div class="right-bottom">
          ...
          <div class="flex bottom-mar">
            <!-- <div class="flex" style="width: 25%;">
							...
							<el-select v-model="fenlei" placeholder="请选择分类">
								<el-option v-for="(item, index) in classList" :key="index" :label="item.name" :value="item.value" />
							</el-select>
						</div> -->
            ...
            <div v-if="$isPc" class="flex" style="padding-left: 2%;width: 35%;">
...
export default {
  ...
  methods: {
    ...
    getTopInfo () {
      let data = {
        loopOr1Equip: '2',
        anaType: 'SheBei-BaoHuSheBei'
      }
      this.$store.dispatch('getTopInfo', data).then(() => {
        let { value: first } = this.topInfo[0]
        let { value: second } = this.topInfo[0].children[0]
        let { value: third } = this.topInfo[0].children[0].children[0]
        this.selectDeviceValue.push(first, second, third)
        this.$store.dispatch('chooseTopInfo', {
          selectDeviceValue: this.selectDeviceValue
        })
        this.getEquipInfo()
        this.getMonitorData()
        this.loadData()
    ...
    getEquipInfo () {
      ...
      this.$store.dispatch('getEquipInfo', data).then(() => {
