<template>
  <div>
    <topHeader equType="2" anaType="SheBei-DianLan" @change="handleTopChange" @topInfoSelect="topInfoSelect">
    </topHeader>
    <div :class="{ flex_row: $isPc, flex_column: !$isPc }">
      <deviceInfo deviceName="电缆" :runningStatus="runningStatus" :communicateStatus="communicateStatus"
        :deviceInfoList="equipAttr" @warnDataBtn="warnDataBtn"></deviceInfo>
      <div class="right_content">
        <tableList class="right-top" title="监测数据" :listContent="listContent" :btnData="btnData" @btnClick="btnClick"
          ...
          <el-table-column v-if="this.lxType == 1" prop="value" label="状态">
            <template slot-scope="scope">
              <div :class="{
        ...
        <div class="right-bottom">
          ...
          <div class="flex bottom-mar">
            ...
            <div v-if="$isPc" class="flex" style="padding-left: 2%;width: 45%;">
              ...
              <el-date-picker v-model="startTime" type="date" placeholder="选择日期" :clearable="false"
                @change="handleDate" />
          ...
          <barCharts barWidth="100%" barHeight="300" :unitL="unitL" :dataList="yunxingData.y" :xName="yunxingData.x"
            :gridPos="gridPos" />
        </div>
      </div>
    </div>
...
import { deviceInfoList, unitData } from '../../../assets/js/config.js'
...
export default {
  ...
  computed: mapState(['topInfo', 'equipInfo', 'dianliujc']),
  data () {
  ...
  methods: {
    ...
    loadData () {
      ...
      let data = {
        siteFid,
        equFid,
        anaType: 'SheBei-DianLan',
        startTime: self.startTime,
        endTime: '',
        isDim: this.dataDim ? this.leixing : '', //max,min,avg
      ...
      self.$store.dispatch('loadData', data).then(res => {
        if (res) {
          this.yunxingData.x = res.data[0].data
          let arr = res.data.filter(i => i.name !== 'showtime')
          for (let i = 0; i < arr.length; i++) {
            arr[i].type = 'line'
            this.yunxingData.y.push(arr[i])
          }
          this.yunxingData.x = res.data[0].data
        } else {
          this.yunxingData = {
    ...
    getEquipInfo () {
      ...
      this.$store.dispatch('getEquipInfo', data).then(res => {
        ...
        this.equipAttr = equipAttr.length > 0 ? equipAttr : deviceInfoList
        ...
        this.dataDim = res.categoryAndDataDim[0].DataDim || null

        this.runningStatus = workAndCommState[0].Values[0][1]
        this.communicateStatus = workAndCommState[0].Values[0][2]
      })
    },
    // 获取监测数据
    getMonitorData () {
