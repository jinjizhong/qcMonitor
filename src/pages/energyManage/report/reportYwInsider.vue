    ...
    <el-dialog top="2vh" style="height:98vh;" title="编辑报告" :visible.sync="editReportVisible" width="70%"
      ...
      <el-form class="edit_form_box" ref="editForm" :model="editForm">
        <div class="ed_common_pd flex_row_center">
          <p style="font-size: 18px; font-weight:bold;" class="ed_common_label">2020年5月裕安大厦运维报告</p>
        </div>
        <div class="flex_row">
        ...
        <p class="editReport_p rd_common_pd">一、系统运行情况</p>
        <p class="ed_common_pd">1.1 电量</p>
        <div class="flex_row ed_bottom">
          <el-form-item class="flex_row editReport_mr_bottom" label="本月用电总量(kVA)：">
            <el-input size="small" style="width: 80%;" v-model="editForm.reportName"></el-input>
          </el-form-item>
          <el-form-item class="flex_row editReport_mr_bottom" label="上月用电总量(kV)：">
            <el-input size="small" style="width: 80%;" v-model="editForm.reportName"></el-input>
          </el-form-item>
        </div>
        <div class="flex_row ed_bottom">
          <el-form-item style="margin-right:20px;" class="editReport_mr_bottom flex_row" label="用电量：">
            <el-input style="width: 550px;" type="textarea" v-model="editForm.reportName"></el-input>
          </el-form-item>
          <el-form-item class="editReport_mr_bottom flex_row" label="分时电量：">
            <el-input style="width: 550px;" type="textarea" v-model="editForm.reportName"></el-input>
          </el-form-item>
        </div>
        <p class="ed_common_pd">1.2 设备运行</p>
        <div v-for="(item,key) in editForm.transfomerList" :key="key">
          <p class="ed_common_pd">{{item.name}}</p>
          <div class="flex_row_yCenter ed_bottom">
            <p style="width: 70px;">负荷：</p>
            <el-input type="textarea" size="small" v-model="item.fuhe"></el-input>
          </div>
          <div class="flex_row_yCenter ed_bottom">
            <p style="width: 70px;">负载率：</p>
            <el-input type="textarea" size="small" v-model="item.fuzailv"></el-input>
          </div>
          <div class="flex_row_yCenter ed_bottom">
            <p style="width: 70px;">温度：</p>
            <el-input type="textarea" size="small" v-model="item.temperature"></el-input>
          </div>
          <div class="flex_row_yCenter ed_bottom">
        ...
        <p class="editReport_p rd_common_pd">二、现场运维</p>
        <p class="ed_common_pd">2.1 站点巡检</p>
        <div class="flex_row ed_bottom">
          <el-form-item class="flex_row editReport_mr_bottom" label="日期：">
            <el-input size="small" style="width: 80%;" v-model="editForm.reportName"></el-input>
          </el-form-item>
        </div>
        <div class="flex_row ed_bottom">
          <p style="width: 80px;">巡检概述：</p>
          <el-input type="textarea" v-model="editForm.reportName"></el-input>
        </div>
        <p class="ed_common_pd">2.2 计划消缺</p>
        <div class="flex_row_yCenter ed_bottom">
          <p style="width: 70px;">小结：</p>
          <el-input type="textarea" size="small" v-model="editForm.reportName"></el-input>
        </div>
        <div>
          <el-table :data="editForm.defectTable" :header-cell-style="{ background: '#F8F8F9' }" style="width: 100%">
            <el-table-column label="缺陷名称">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.defectName"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="relevanceEquip" label="关联设备">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.relevanceEquip"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="defectLevel" label="缺陷等级">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.defectLevel"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="planDate" label="计划检修日期">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.planDate"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="finishStatus" label="完成状态">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.finishStatus"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="implementation" label="执行情况">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.implementation"></el-input>
              </template>
            </el-table-column>
            <el-table-column width="80" label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="danger" @click="handleEditDelete(scope.$index, scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div @click="addNewLine" class="add_new_line flex_row_center">
            <img style="margin-right:3px;" width="18" height="18" src="../../../assets/image/report/addCircle.png"
              alt="img">
            <p>新增一行</p>
          </div>
        </div>
        <p class="ed_common_pd">2.3 故障抢修</p>
        <div class="flex_row_yCenter ed_bottom">
          <p style="width: 70px;">小结：</p>
          <el-input type="textarea" size="small" v-model="editForm.reportName"></el-input>
        </div>
        <div>
          <el-table :data="editForm.repairTable" :header-cell-style="{ background: '#F8F8F9' }" style="width: 100%">
            <el-table-column prop="faultFunc" label="故障事件">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.defectName"></el-input>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="faultTime" label="故障时间">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.faultTime"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="presentTime" label="到场时间">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.presentTime"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="faultRecoverTime" label="故障恢复时间">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.faultRecoverTime"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="finishStatus" label="完成情况">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.finishStatus"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="accidentAnalyze" label="事故分析">
              <template slot-scope="scope">
                <el-input size="small" v-model="scope.row.accidentAnalyze"></el-input>
              </template>
            </el-table-column>
            <el-table-column width="80" label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="danger" @click="handleEditDeleteRepair(scope.$index, scope.row)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
          <div @click="addNewLineRepair" class="add_new_line flex_row_center">
            <img style="margin-right:3px;" width="18" height="18" src="../../../assets/image/report/addCircle.png"
              alt="img">
            <p>新增一行</p>
          </div>
